# Проектирование test-rules.cjs

> Claude-эквивалент [`.cursor/rules/js-dev.mdc`](../../.cursor/rules/js-dev.mdc). Применяется при работе с `test-rules.cjs`.
...
- **Const-хелпер** в test — одна операция (`lintStrings`, `getFiredRules`)
...
- Выносить хелпер в test — если **≥3 повтора** в `test-rules.cjs` (не для `src/domain/` — там см. [ts-dev.md](ts-dev.md))

## Test-only domain imports

`test-rules.cjs` намеренно импортирует из `domain/` напрямую (не через `markdownlint-hlprs.js`):
...
Top-level `const`-хелперы и вложенные `const`-стрелки — **callee перед caller** (как в [ts-dev.md](ts-dev.md)). Проверка: `npm run check:order` (входит в `npm test` и `npm run check`). Scope: `src/**/*.ts`, `test-rules.cjs`.
...
2. ≥3 повтора в `test-rules.cjs` и меняются вместе — или функция стала двусмысленной
