...
`targetToGlobs` в [`lint-markdown.cjs`](../../bin/lint-markdown.cjs) принимает путь и в чужом для текущей платформы формате: без интерактивного диалога выбора — только конвертация строки пути через `wslpath`.
