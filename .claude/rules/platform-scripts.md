...
## Кросс-запуск путей (Windows ↔ WSL)

`targetToGlobs` в [`lint-markdown.cjs`](../../bin/lint-markdown.cjs) принимает путь и в чужом для текущей платформы формате — аналогично `pick_directory()` / `_wslpath` в `fs-tools/shared/picker.py` (другой проект пользователя), но без диалога: только конвертация строки пути через `wslpath`.

- `abs = path.resolve(repoRoot, target)` не существует → пробуем `crossPlatformPath(target)`:
  - на `win32`, если `target` похож на WSL-путь (`/...`) — `wsl wslpath -w <target>` (через `wsl.exe`, вызывающий `wslpath` внутри дистрибутива); `~/...` не поддерживается — `wslpath` не разворачивает tilde без shell (проверено: возвращает путь как есть с кодом 0, ложноположительный результат);
  - в WSL (Linux + `/proc/version` содержит `microsoft`, либо `WSL_DISTRO_NAME` в env), если `target` похож на Windows-путь (`C:\...`, `C:/...`, `\\...` UNC) — `wslpath -u <target>`.
- Конвертация — `null` при отсутствии `wsl`/`wslpath`, ненулевом коде или пустом выводе; тогда — исходная ошибка `Path not found`.
- Не поддерживается: конвертация на обычном Linux/macOS (там нет `wslpath`) — `crossPlatformPath` возвращает `null`, путь ищется только как есть.
...
## Веб-хук уведомлений (`notify.js`)

[`bin/lint-markdown.cjs`](../../bin/lint-markdown.cjs) после неуспешного прогона (`res.status` ≠ 0) вызывает `sendWebhook` из [`src/notify.ts`](../../src/notify.ts) (артефакт — `notify.js` в корне; см. [ts-dev.md](ts-dev.md)):

- Конфигурация — из `.env` в корне репозитория (`MDLINT_WEBHOOK_URL` / `MDLINT_WEBHOOK_TOK`); шаблон — [`.env.example`](../../.env.example); без `MDLINT_WEBHOOK_URL` — тихий no-op.
- URL обязан быть `https://`; иначе токен не отправляется.
- Тело — короткое сообщение об ошибке (`{"text": "markdownlint-custom: ошибка lint"}`), не список нарушений (детали — в выводе `markdownlint-cli2`).
- Таймаут — `2000` мс (`AbortController`); fire-and-forget — ошибка/таймаут не влияет на код возврата CLI.
- Только `bin/lint-markdown.cjs`; не подключать к другим точкам входа без явного запроса.
