...
```bash
npm install
npm test        # pretest → build, tests/run-all + tests/test-cli2-config + check-function-order
```
...
| [`tests/`](tests/) (`run-all.cjs`, `helpers.cjs`, `examples.test.cjs`, `hlprs.test.cjs`, `rules/*.test.cjs`, `test-cli2-config.cjs`, `test-markdownlint-ignore.cjs`), [`check-function-order.cjs`](check-function-order.cjs) | Тесты (1 файл на custom lint-правило), проверка cli2-конфига и игнор-файла, порядок функций |
...
| `npm test` | `pretest` (build) + `tests/run-all.cjs` + `tests/test-cli2-config.cjs` + `tests/test-markdownlint-ignore.cjs` + `check-function-order.cjs` (cli2 parity — только здесь) |
...
| `npm run check` | `precheck` (build) + `tsc --noEmit` + `node --check` на 20 `.js`/`.cjs` + порядок функций (без запуска `test-cli2-config`) |
