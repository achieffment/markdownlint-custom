...
```bash
npm install
npm test        # pretest → build, tests/run-all + tests/test-cli2-config + tests/test-markdown-tables + check-function-order
```
...
| [`tests/`](tests/) (`run-all.cjs`, `helpers.cjs`, `examples.test.cjs`, `hlprs.test.cjs`, `rules/*.test.cjs`, `test-cli2-config.cjs`, `test-markdownlint-ignore.cjs`, `test-markdown-tables.cjs`), [`check-function-order.cjs`](check-function-order.cjs) | Тесты (1 файл на custom lint-правило), проверка cli2-конфига, игнор-файла и выравнивания таблиц, порядок функций                             |
...
| `npm test`                 | `pretest` (build) + `tests/run-all.cjs` + `tests/test-cli2-config.cjs` + `tests/test-markdownlint-ignore.cjs` + `tests/test-markdown-tables.cjs` + `check-function-order.cjs` (cli2 parity — только здесь)               |
...
| `npm run check`            | `precheck` (build) + `tsc --noEmit` + `node --check` на 21 `.js`/`.cjs` + порядок функций (без запуска `test-cli2-config`)                                                                                               |
